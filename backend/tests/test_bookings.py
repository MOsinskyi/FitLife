...
def test_create_booking_as_customer(client, customer_token, sample_customer, sample_schedule):
    ...
    response = client.post(
        ...
        json={"customer_id": sample_customer["id"], "schedule_id": sample_schedule["id"]},
    )
...
def test_get_bookings_list(client, customer_token, sample_customer, sample_schedule):
    ...
    # Then get the list
    response = client.get("/api/v1/bookings", headers={"Authorization": f"Bearer {customer_token}"})
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
...
def test_get_booking_by_id(client, customer_token, sample_customer, sample_schedule):
    ...
    # Get the booking
    response = client.get(
        f"/api/v1/bookings/{booking_id}", headers={"Authorization": f"Bearer {customer_token}"}
    )
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
...
def test_get_nonexistent_booking(client, customer_token):
    ...
    response = client.get(
        "/api/v1/bookings/nonexistent-id", headers={"Authorization": f"Bearer {customer_token}"}
    )
...
def test_booking_nonexistent_schedule(client, customer_token, sample_customer):
    ...
    response = client.post(
        "/api/v1/bookings",
        headers={"Authorization": f"Bearer {customer_token}"},
        json={"customer_id": sample_customer["id"], "schedule_id": "nonexistent-schedule-id"},
    )

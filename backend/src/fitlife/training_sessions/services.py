...
class TrainingSessionService:
    ...
    async def create_training_session(
        self, data: TrainingSessionCreateSchema
    ) -> TrainingSession:
        ...
        created_session = (
            await self._training_session_repository.create_training_session(
                training_session
            )
        )
        ...
        if data.participants:
            await self.add_participants_to_session(
                created_session.id, data.participants
            )

        return await self._training_session_repository.get_training_session_by_id(
            created_session.id
        )

    async def get_training_session(self, session_id: UUID) -> TrainingSession:
        training_session = (
            await self._training_session_repository.get_training_session_by_id(
                session_id
            )
        )
    ...
    async def update_training_session(
        self, session_id: UUID, data: TrainingSessionUpdateSchema
    ) -> TrainingSession:
        training_session = (
            await self._training_session_repository.get_training_session_by_id(
                session_id
        ...
        await self._training_session_repository.update_training_session(
            session_id, data
        )
    ...
    async def add_participants_to_session(
        self, session_id: UUID, member_ids: list[UUID]
    ) -> TrainingSession:
        training_session = (
            await self._training_session_repository.get_training_session_by_id(
                session_id
        ...
        for member_id in member_ids:
            ...
            is_already_participant = any(
                p.member_id == member_id for p in training_session.participants
            )
            if is_already_participant:
                raise ValueError(
                    f"Member with id {member_id} is already participating in this session"
                )
    ...
    async def remove_participant_from_session(
        self, session_id: UUID, member_id: UUID
    ) -> TrainingSession:
        training_session = (
            await self._training_session_repository.get_training_session_by_id(
                session_id
        ...
        if not participant_to_remove:
            raise ValueError(
                f"Member with id {member_id} is not participating in this session"
            )

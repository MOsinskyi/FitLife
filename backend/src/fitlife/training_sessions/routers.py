...
training_session_router = APIRouter(
    prefix="/training-sessions", tags=["🏋️ Training Sessions"]
)
...
async def create_training_session(
    data: TrainingSessionCreateSchema, service: TrainingSessionServiceDep
):
    try:
...
async def update_training_session(
    session_id: UUID,
    data: TrainingSessionUpdateSchema,
...
async def add_participants(
    session_id: UUID, member_ids: list[UUID], service: TrainingSessionServiceDep
):
    try:
...
async def remove_participant(
    session_id: UUID, member_id: UUID, service: TrainingSessionServiceDep
):
    try:

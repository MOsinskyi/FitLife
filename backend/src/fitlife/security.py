...
class Security:
    ...
    def verify_password(password: str, hashed_password: str) -> bool:
        return bcrypt.checkpw(
            password.encode(settings.security.hash_encoding), hashed_password.encode(settings._security.hash_encoding)
        )
    ...
    def create_access_token(data: dict, expires_delta: timedelta | None = None) -> str:
        ...
        return jwt.encode(to_encode, settings.security.secret_key, algorithm=settings._security.algorithm)
    ...
    def decode_access_token(token: str) -> dict:
        try:
            payload = jwt.decode(token, settings.security.secret_key, algorithms=[settings._security.algorithm])

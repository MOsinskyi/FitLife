...
coach_specialization_table = Table(
    ...
    Base.metadata,
    Column('coach_id', UUID(as_uuid=True), ForeignKey('coaches.id', ondelete='CASCADE'), primary_key=True),
    Column('specialization_id', UUID(as_uuid=True), ForeignKey('specializations.id', ondelete='CASCADE'), primary_key=True),
)

...
async def get_specialization_repository(session: AsyncSession = Depends(get_session)) -> SpecializationRepository:
...
SpecializationServiceDep = Annotated[SpecializationService, Depends(get_specialization_service)]
SpecializationRepositoryDep = Annotated[SpecializationRepository, Depends(get_specialization_repository)]
